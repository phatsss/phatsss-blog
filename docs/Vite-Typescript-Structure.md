...
[**Vite-TS-Structure on Github**](https://github.com/phatsss/vite-ts-structure.git) - A secure, production-ready Vite + React + TypeScript project with SOLID-compliant architecture, robust form validation.

# SOLID Project Structure

A React + Vite + TanStack Query + TanStack Router application with SOLID-compliant architecture, robust form validation, and comprehensive permission management.
...
This project uses [shadcn/ui](https://ui.shadcn.com/) for beautifully designed, accessible UI components. shadcn/ui is not a component library but a collection of reusable components built using Radix UI and Tailwind CSS.
...
Components are added to the `src/components/ui` directory and can be customized to fit your needs. The configuration is managed in `components.json`.

### Styling Components

All components use CSS variables for theming, which can be customized in `src/styles.css`. The project uses the "New York" style variant with neutral as the base color.

## Internationalization (i18n)

This project uses [i18next](https://www.i18next.com/) for internationalization. The configuration is in `src/i18n/config.ts`.

### Adding Translations

Translations are stored in JSON files in the `src/i18n/locales` directory. To add a new language:

1. Create a new JSON file in the `src/i18n/locales` directory (e.g., `fr.json`)
2. Add the language to the resources in `src/i18n/config.ts`

### Using Translations

Use the `useI18n` hook to access translations in your components:
...
This project includes a comprehensive permission system for role-based access control.

### Role-Based Access Control

Use the `RoleBasedAccess` component to conditionally render UI elements based on user roles or permissions:
...
Use the `usePermissions` hook to check permissions in your components:
...
This project uses [DnD Kit](https://dndkit.com/) for drag and drop functionality.
...
This project uses [Recharts](https://recharts.org/) for data visualization.
